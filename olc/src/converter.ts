...
const createLink = (dest: LinkType, originalLink: string, altOrBlockRef: string, sourceFile: TFile, plugin: LinkConverterPlugin): string => {
	// console.log(plugin.app.vault.getAbstractFileByPath(originalLink));
    ...
    if (file) finalLink = getFileLinkInFormat(file, sourceFile, plugin, 'absolute-path');
    ...
    if (dest === 'wiki') {
        ...
        return `[${originalLink}](${encodeURI(finalLink)}#${encodedBlockRef})`;
        return `[${originalLink}](${encodeURI(finalLink)}${fileExtension}#${encodedBlockRef})`;
    }
